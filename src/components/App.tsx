// src/components/App.tsx (VERSIONE FINALE DEBUGGATA)
...
// +++ FUNZIONE HELPER ROBUSTA PER CHIAMARE LA NOSTRA API (CON FIX) +++
async function callApi(type: 'chat' | 'generateJson', payload: any) {
    ...
    // +++ MODIFICA CHIAVE QUI +++
    // Prima di fare il parse, controlliamo se la risposta è una stringa vuota.
    // Se lo è, restituiamo un oggetto vuoto per evitare il crash.
    if (responseText.trim() === '') {
        console.warn("Il server ha restituito una risposta 200 OK ma vuota. Restituisco un oggetto vuoto.");
        return {};
    }
...
// Funzione helper per estrarre il testo
...
export const App = () => {
    ...
    const createPlayerCharacter = async (userMessage: ChatMessage) => {
        try {
            ...
            if (!jsonStr) {
                // Se non c'è JSON, non possiamo creare il personaggio.
                throw new Error("Il DM non ha fornito i dati del personaggio. Prova a descriverlo di nuovo.");
            }
    ...
    const prepareBattle = async () => {
        ...
        try {
            ...
            if (!jsonStr) {
                throw new Error("Il DM non ha fornito i dati per la battaglia.");
            }
